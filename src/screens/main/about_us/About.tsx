...
const About = () => {
  ...
  const [data, setData] = useState<{about_us: string}>({about_us: ''});
  ...
  // GETTING HTML DATA FOR ABOUT US
  const getSettingData = async () => {
    try {
      setIsLoading(true);
      const res = await gettingSettingsData();
      if (res.success) {
        console.log('res of about us data:', res.data.data.about_us);
        setData({about_us: res.data.data.about_us});
      } else {
      ...
      <View style={GLOBALSTYLE.container}>
        <SafeAreaView>
          <View style={GLOBALSTYLE.flex}>
            ...
            <Text style={GLOBALSTYLE.authTitle_small}>About Us</Text>
            <Text></Text>
          </View>
          <ScrollView
